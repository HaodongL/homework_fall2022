...
class IQLAgent(DQNAgent):
    ...
    def train(self, ob_no, ac_na, re_n, next_ob_no, terminal_n):
        ...
        if (self.t > self.learning_starts
            ...
            new_std = np.std(pred_error)
            old_std = self.running_rnd_rew_std
            self.running_rnd_rew_std = self.rnd_gamma*new_std + (1-self.rnd_gamma)*old_std
            expl_bonus = normalize(pred_error, np.mean(pred_error), self.running_rnd_rew_std)
            expl_reward = re_n + expl_bonus
            ...
            mixed_reward = explore_weight * expl_reward + exploit_weight * env_reward
            ...
            exploitation_critic_loss_v = self.exploitation_critic.update_v(ob_no, ac_na)
            exploitation_critic_loss_q = self.exploitation_critic.update_q(ob_no, ac_na, 
            ...
            log['Exploitation Critic V Loss'] = exploitation_critic_loss_q['Training Q Loss']
            log['Exploitation Critic Q Loss'] = exploitation_critic_loss_v['Training V Loss']
